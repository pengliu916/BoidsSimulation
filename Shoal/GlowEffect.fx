...
static float Gauss[NUMWT] = { 0.98, 0.9, 0.8, 0.7, 0.5, 0.3, 0.1, 0.05, 0.01 };
#define WT_NORMALIZE (1.0/(1.0+2.0*(0.98 + 0.9 + 0.8 + 0.7 + 0.5 + 0.3 + 0.1 + 0.05 + 0.01)))
//--------------------------------------------------------------------------------------
// Constants
//--------------------------------------------------------------------------------------
...
float4 PS_Glow_V(PS_INPUT input) : SV_Target
{
	...
	float4 c2;
	float4 c = H_glowTex.Sample(samGeneral, currentLocation) * (WT_NORMALIZE);
	float2 step = float2( 0, 1.0f / DepthHeight);
	float2 dir = step;
	for(int i=0; i<NUMWT; i++)
	{
		c2 = H_glowTex.Sample(samGeneral, currentLocation + dir);
...
float4 PS_Glow_H(PS_INPUT input) : SV_Target
{
	...
	float4 c2;
	float4 c = downSampledTex.Sample(samGeneral, currentLocation) * (WT_NORMALIZE);
	float2 step = float2( 1.0f / DepthWidth, 0 );
	float2 dir = step;
	for(int i=0; i<NUMWT; i++)
	{
		c2 = downSampledTex.Sample(samGeneral, currentLocation + dir);
		c += c2 * (Gauss[i]*WT_NORMALIZE);
		c2 = downSampledTex.Sample(samGeneral, currentLocation - dir);
		c += c2 * (Gauss[i]*WT_NORMALIZE);
		dir += step;
...
float4 PS_Glow_ALL(PS_INPUT input) : SV_Target
{
	...
	HV_glowTex.Sample(samGeneral, 0.5*input.Tex / float2(DepthWidth, DepthHeight)) *blend_factor;
	color.a=1;
	return color;
}
