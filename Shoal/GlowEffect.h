...
public:
	...
	ID3D11PixelShader*				m_pPixelShader_UpDownSample;
	...
	ID3D11Texture2D*				m_pUpDownSampledTexture2D;
	...
	ID3D11RenderTargetView*			m_pUpDownSampledTextureRTV;
	...
	ID3D11ShaderResourceView*		m_pUpDownSampledTextureSRV;
	...
	GlowEffect(UINT width = SUB_TEXTUREWIDTH, UINT height = SUB_TEXTUREHEIGHT)
	{
		...
		m_renderGlowTargetWidth = (UINT)(m_rendertargetWidth / 2.0);
		m_renderGlowTargetHeight = (UINT)(m_rendertargetHeight / 2.0);
		...
		ID3D11ShaderResourceView* pInputTextureSRV, CModelViewerCamera* pCamera)
	{
		...
		V_RETURN(pd3dDevice->CreatePixelShader(pPSBlob->GetBufferPointer(), pPSBlob->GetBufferSize(), NULL, &m_pPixelShader_UpDownSample));
		...
		V_RETURN(pd3dDevice->CreateTexture2D(&RTtextureDesc, NULL, &m_pUpDownSampledTexture2D));
		V_RETURN(pd3dDevice->CreateTexture2D(&RTtextureDesc, NULL, &m_pOutputTexture2D));

		RTshaderResourceDesc.Format=RTtextureDesc.Format;
		V_RETURN(pd3dDevice->CreateShaderResourceView(m_pUpDownSampledTexture2D, &RTshaderResourceDesc, &m_pUpDownSampledTextureSRV));
		V_RETURN(pd3dDevice->CreateShaderResourceView(m_pOutputTexture2D, &RTshaderResourceDesc, &m_pOutputTextureSRV));
		
		RTviewDesc.Format=RTtextureDesc.Format;
		V_RETURN(pd3dDevice->CreateRenderTargetView(m_pUpDownSampledTexture2D, &RTviewDesc, &m_pUpDownSampledTextureRTV));
	...
	void Release()
	{
		...
		SAFE_RELEASE(m_pPixelShader_UpDownSample);
		...
		SAFE_RELEASE(m_pUpDownSampledTexture2D);
		SAFE_RELEASE(m_pUpDownSampledTextureRTV);
		SAFE_RELEASE(m_pUpDownSampledTextureSRV);
	...
	void Render(ID3D11DeviceContext* pd3dImmediateContext)
	{
			...
			pd3dImmediateContext->OMSetRenderTargets(1, &m_pUpDownSampledTextureRTV, NULL);
			pd3dImmediateContext->PSSetShader(m_pPixelShader_UpDownSample, NULL, 0);
			...
			pd3dImmediateContext->PSSetShaderResources(1, 1, &m_pUpDownSampledTextureSRV);
			pd3dImmediateContext->Draw(1,0);	
			...
			pd3dImmediateContext->PSSetShaderResources(2, 1, &m_pGlow_H_TextureSRV);
			pd3dImmediateContext->Draw(1,0);	
			...
			pd3dImmediateContext->PSSetShaderResources(3, 1, &m_pGlow_HV_TextureSRV);
			pd3dImmediateContext->Draw(1,0);
